...
public class FishDrawable extends Drawable {
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        PointF smallCenterPoint = makeSegment(canvas,bodyBottomCenterPoint,
                BIG_CIRCLE_RADIUS,MIDDLE_CIRCLE_RADIUS,FIND_MIDDLE_CIRCLE_LENGTH,
                fishAngle,true);
        //画小节支
        makeSegment(canvas,smallCenterPoint,MIDDLE_CIRCLE_RADIUS,SMALL_CIRCLE_RADIUS,FIND_SMALL_CIRCLE_LENGTH,
                fishAngle,false);

        float findEdgeLength = (float) (Math.abs(Math.sin(Math.toRadians(currentValue * frequance)) *
                BIG_CIRCLE_RADIUS));
        //画尾巴
    ...
    private void makeTriangle(Canvas canvas, PointF startPoint,float findTriangle,
                              float bigCircleLength, float fishAngle) {
        float tr = 0;
